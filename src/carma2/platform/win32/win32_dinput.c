...
C2_HOOK_VARIABLE_IMPLEMENT(JOYINFOEX, gJoy1_info, 0x006ac7e8x);
...
void C2_HOOK_FASTCALL ResetDinputJoystickFFB(int index) {
    IDirectInputDevice2A *device = C2V(gDirectInputJoystickDevices)[index];
    if (device != NULL && (C2V(gJoystickFFB) & (1 << index))) {
        IDirectInputDevice2_SendForceFeedbackCommand(device, DISFFC_STOPALL);
    }
}
C2_HOOK_FUNCTION(0x00457ff0, ResetDinputJoystickFFB)

int C2_HOOK_FASTCALL UnloadDinputFFBEffectWithName(const char *name) {
    int i;
...
C2_HOOK_FUNCTION(0x00458060, UnloadDinputFFBEffectWithName)
