...
void C2_HOOK_FASTCALL InitOpponents(tRace_info* pRace_info) {
    ...
    C2V(gProgram_state).current_car.field_0x155c = -1; /* tS16? */
    C2V(gHead_on_cos_value) = cosf(.5235668420791626f);
    ...
    for (i = 0, opponent_number = 0; i < C2V(gProgram_state).AI_vehicles.number_of_opponents; i++, opponent_number++) {
        ...
        C2V(gProgram_state).AI_vehicles.opponents[i].repair_interval = IRandomBetween(C2V(gMinTimeOpponentRepair), C2V(gMaxTimeOpponentRepair));
        C2V(gProgram_state).AI_vehicles.opponents[i].last_repair_time = C2V(gTime_stamp_for_this_munging);
        oppo_dprintf("*** Opponent %s, repair_interval is %d", C2V(gProgram_state).AI_vehicles.opponents[i].car_spec->driver_name,
        ...
        StunTheBugger(&C2V(gProgram_state).AI_vehicles.opponents[i], -1);
    }
    ...
    if (opponents_left != 0) {
        if (C2V(gCurrent_race).race_spec->opponent_nastiness_level < 10.f) {
            ...
            for (i = 0; i < count_type_2; i++) {
                ...
                rstart = IRandomBetween(0, opponents_left - 1);
                for (r = rstart;;) {
            ...
            count_type_1 = (int)(0.2f * count_type_2);
            for (i = 0; i < count_type_1; i++) {
                int rstart;
                int r;

                rstart = IRandomBetween(0, opponents_left - count_type_2 - 1);
                for (r = rstart;;) {
                    if (C2V(gProgram_state).AI_vehicles.opponents[r].field_0xb4 == 0) {
