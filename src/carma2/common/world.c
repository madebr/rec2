...
void C2_HOOK_FASTCALL ApplyMaterialCallbackOnAllMaterials(br_model* pModel, material_cbfn* pCallback) {
    if (pModel->faces == NULL) {
        ...
        if (v11m != NULL) {
            ...
            for (i = 0; i <  v11m->ngroups; i++) {
                v11group* v11g = &v11m->groups[i];
                if (*(br_material**)v11g->face_colours != NULL) {
                    pCallback(*(br_material**)v11g->face_colours);
                }
            }
        }
    } else {
...
intptr_t C2_HOOK_CDECL ApplyMaterialCallbackOnAllModelActorsCallback(br_actor* pActor, material_cbfn* pCallback) {
    ...
    return BrActorEnum(pActor, ApplyMaterialCallbackOnAllModelActorsCallback, pCallback);
}
...
void C2_HOOK_FASTCALL LoadSmashableTrackEnvironment(FILE* pF, const char* pPath) {
    ...
    for (i = 0; i < C2V(gCount_track_smashable_environment_specs); i++) {
        ...
        case kSmashableMode_TextureChange:
            ...
            spec->mode_data.texture_change.field_0x44 = -1;
...
void C2_HOOK_FASTCALL LoadTrack(const char* pFile_name, tTrack_spec* pTrack_spec, tRace_info* pRace_info) {
    ...
    c2_sprintf(race_fixup_filename, "%s%s%s%s", local_race_path, C2V(gRenderer_fixup_basename), C2V(gRenderer_fixup_extension));
    ...
    if (f != NULL) {
        ...
        while (1) {
            ...
            matexc->texture_name = BrMemAllocate(c2_strlen(str), kMem_misc_string);
    ...
    GetThreeFloats(f, &pRace_info->initial_position.v[0], &pRace_info->initial_position.v[1], &pRace_info->initial_position.v[2]);
    ...
    for (i = 0; i < pRace_info->check_point_count; i++) {
        ...
        GetThreeInts(f, &pRace_info->checkpoints[i].timer_increments[0], &pRace_info->checkpoints[i].timer_increments[1], &pRace_info->checkpoints[i].timer_increments[2]);
        ...
        for (j = 0; j < pRace_info->checkpoints[i].count_quads; j++) {
            ...
            BrVector3Sub(&tmp2,
                ...
                &pRace_info->checkpoints[i].quads[j].points[0]);
            BrVector3Cross(&pRace_info->checkpoints[i].normals[j], &tmp1, &tmp2);
        }
        BrVector3InvScale(&pRace_info->checkpoints[i].pos, &pRace_info->checkpoints[i].pos, 1.f / (4 * (float)pRace_info->checkpoints[i].count_quads));
    }
    ...
    for (i = 0; i < C2V(gTrack_storage_space).models_count; i++) {
        br_model* model;
        int* model_index_buffer;
        int** face_index_buffer;

        PossibleService();
        model = C2V(gTrack_storage_space).models[i];
        model_index_buffer = BrMemAllocate((model->nvertices + 1) * sizeof(int), BR_MEMORY_APPLICATION);
        face_index_buffer = BrMemAllocate((model->nfaces * 3) * sizeof(int), BR_MEMORY_APPLICATION);

        for (j = 1; j < model->nvertices; j++) {
            if (model_index_buffer[j] == 0) {
                int k;

                model_index_buffer[j] = j;
                for (k = j + 1; k < model->nvertices; k++) {
                    br_vector3 d;

                    BrVector3Sub(&d, &model->vertices[j].p, &model->vertices[k].p);
                    if (BrVector3LengthSquared(&d) < 1e-8f) {
                        model_index_buffer[k] = j;
                    }
                }
            }
        }
        for (j = 0; j < model->nfaces; j++) {
            ...
            for (k = 0; k < 3; k++) {
                face_index_buffer[j][k] = model_index_buffer[1+model->faces[j].vertices[k]];
            }
        }
        if (model->nfaces > 1) {
                    ...
                    for (l = 0; l < 3; l++) {
                        ...
                        for (m = 0; m < 3; m++) {
                            ...
                            if ((s1a == s2a && s1b == s2b) || (s1a == s2b && s1b == s2a)) {
                                ...
                                BrVector3Sub(&d21, &model->vertices[model->faces[k].vertices[s2b]].p, &model->vertices[model->faces[k].vertices[s2a]].p);
                                s1c = face_index_buffer[j][(l + 2) % 3];
                                s2c = face_index_buffer[k][(m + 2) % 3];
                                BrVector3Sub(&o31b, &model->vertices[model->faces[k].vertices[s2c - 1]].p, &model->vertices[model->faces[k].vertices[s2a - 1]].p);
                                BrVector3Sub(&o31a, &model->vertices[model->faces[j].vertices[s1c - 1]].p, &model->vertices[model->faces[j].vertices[s2c - 1]].p);
    ...
    GetPairOfInts(f, &C2V(gProgram_state).default_depth_effect.start, &C2V(gProgram_state).default_depth_effect.end);
    ...
    /* START OF FUNK */
    GetALineAndDontArgue(f, s);
    AddFunkotronics(f, -2, 1080, NULL);

    /* START OF GROOVE */
    GetALineAndDontArgue(f, s);
    AddGroovidelics(f, -2, C2V(gUniverse_actor), 1080, 0);
    ...
    for (i = 0; i < C2V(gTrack_storage_space).models_count; i++) {
            ...
            if (j != C2V(gTree_surgery_pass1_count)) {
                for (j = 0; j < C2V(gTree_surgery_pass2_count); j++) {
                    if (c2_strcmp(model->identifier, C2V(gTree_surgery_pass2)->original) == 0) {
                        ...
                        replacement = LoadModel(C2V(gTree_surgery_pass2)->replacement);
                        if (replacement != NULL) {
        ...
        if (model != NULL && (model->flags & (BR_MODF_KEEP_ORIGINAL | BR_MODF_UPDATEABLE))) {
            ...
            for (j = 0; j < V11MODEL(model)->ngroups; j++) {
                ...
                (br_material*)v11_group->face_colours = model->faces[*v11_group->face_user].material;
                if ((br_material*)v11_group->face_colours != NULL) {
                    SetMaterialTrackLighting((br_material*)v11_group->face_colours);
                    if (((br_material*)v11_group->face_colours)->index_shade == NULL) {
                        ((br_material*)v11_group->face_colours)->index_shade = BrTableFind("DRRENDER.TAB");
                        BrMaterialUpdate((br_material*)v11_group->face_colours, BR_MATU_ALL);
                    }
                }
            }
    ...
    for (i = 0; i < count_material_modifiers; i++) {
        ...
        if (c2_strcmp(str, "NONE") == 0) {
            ...
            c2_strcpy(&str[c2_strlen(".PIX")], ".PIX");
            LoadSinglePixelmap(&C2V(gTrack_storage_space), str);
            c2_strcpy(&str[c2_strlen(".MAT")], ".MAT");
    ...
    for (i = 0; i < C2V(gProgram_state).track_spec.count_non_cars; i++) {
        ...
        idx = C2V(gNon_car_indices)[10 * actor->identifier[1] + actor->identifier[2]] + 40 - 1;
    ...
    for (i = 0; i < pRace_info->count_network_start_points; i++) {
        GetThreeScalars(f, &pRace_info->net_starts[i].pos.v[0], &pRace_info->net_starts[i].pos.v[1], &pRace_info->net_starts[i].pos.v[2]);
...
C2_HOOK_FUNCTION(0x00504bf0, LoadTrack)
