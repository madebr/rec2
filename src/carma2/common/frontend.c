...
static void C2_HOOK_FASTCALL LoadMenuImages(void) {
    ...
    for (i = 0; i < count; i++) {
        ...
        c2_strcpy(s, s2);
        c2_strcat(s, ".TIF");
        C2V(gFrontend_images)[i + 8] = DRLoadPixelmap(s);
    }
...
void C2_HOOK_FASTCALL CreateMenuActors(void) {
    ...
    C2V(gFrontend_backdrop0_material_prims)[2].t = 0;
    ...
    C2V(gFrontend_backdrop1_material_prims)[2].t = 0;
    ...
    C2V(gFrontend_backdrop2_material_prims)[2].t = 0;
...
static void C2_HOOK_FASTCALL LoadMenuModels(void) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (i == 0) {
            C2V(gFrontend_backdrop_actors)[0]->model = BrModelLoad(s);
    ...
    for (i = 0; i < count; i++) {
        ...
        if (i == 0) {
            C2V(gFrontend_backdrop_actors)[1]->model = BrModelLoad(s);
    ...
    for (i = 0; i < count; i++) {
        ...
        if (i == 0) {
            C2V(gFrontend_backdrop_actors)[2]->model = BrModelLoad(s);
...
void C2_HOOK_FASTCALL FRONTEND_CreateItemBrenderObjects(tFrontend_brender_item* pFrontend_brender_item, tS16 pX, tS16 pY, tS16 pWidth, tS16 pHeight, br_colour pColour, br_pixelmap* pMap, const char* pText) {
    ...
    if (pMap != NULL) {
        pFrontend_brender_item->field_0x10 = BrPixelmapAllocate(BR_PMT_ARGB_4444,pMap->width, pMap->height, NULL, 0);
        BrPixelmapCopy(pFrontend_brender_item->field_0x10, pFrontend_brender_item->field_0x10);
        pFrontend_brender_item->field_0xc = BrPixelmapAllocate(BR_PMT_ARGB_4444, pMap->width, pMap->height, NULL, 0);
        BrPixelmapCopy(pFrontend_brender_item->field_0xc, pFrontend_brender_item->field_0x10);
    } else {
        pFrontend_brender_item->field_0x10 = BrPixelmapAllocate(BR_PMT_ARGB_4444, 8, 8, NULL, 0);
        BrPixelmapFill(pFrontend_brender_item->field_0x10, BR_COLOUR_RGBA(0, 0, 0x80, 0));
        pFrontend_brender_item->field_0xc = BrPixelmapAllocate(BR_PMT_ARGB_4444, 8, 8, NULL, 0);
        BrPixelmapCopy(pFrontend_brender_item->field_0xc, pFrontend_brender_item->field_0x10);
    ...
    pFrontend_brender_item->prims[2].t = 0;
...
int C2_HOOK_FASTCALL FRONTEND_CreateMenu(tFrontend_spec* pFrontend_spec) {
    ...
    for (i = 0; i < pFrontend_spec->count_items; i++) {
        ...
        FRONTEND_CreateItemBrenderObjects(&C2V(gFrontend_brender_items)[C2V(gFrontend_count_brender_items)],
            item->x,
            y,
            width,
            height,
