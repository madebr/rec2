...
C2_HOOK_VARIABLE_IMPLEMENT(tPhysicsError_cbfn*, gPhysics_error_cb, 0x004c61f0);
...
static tPlane_side calculate_plane_side(br_vector4* pPlane, br_vector3* pPoint) {
    ...
    s = BrVector3Dot(pPlane, pPoint) + pPlane->v[3];
    if (s < -1e6f) {
...
int C2_HOOK_FASTCALL PolyhedronCollisionShape_AddPoint(tCollision_shape_polyhedron_data* pPolyhedron, br_vector3* pVertex, tU8 pVertex_index, tPolyhedron_edge_indexes* pEdge_to_plane_indices) {
    ...
    for (i = 0; i < pPolyhedron->count_planes; i++) {
        ...
        all_positive &= ~plane_side_buffer[i] & 1;
    }
    ...
    for (i = 0; i < pPolyhedron->count_edges; i++) {
        ...
        if (((side1 ^ side2) & ePlaneSide_negative) != ePlaneSide_positive) {
            /* Exactly one plane of edge has vertex on negative side */
            if ((side1 & ePlaneSide_zero) == 0 && (side2 & ePlaneSide_zero) == 0) {
                br_vector3* v1,* v2;
                br_vector3 tv1, tv2;
                ...
                if (BrVector3Dot(&pPolyhedron->planes[plane1], &pPolyhedron->planes[plane1]) * BrVector3Dot(new_plane, &pPolyhedron->planes[plane2])
                    ...
                    BrVector3Negate(new_plane, new_plane);
                }
                ...
                if ((side1 & ePlaneSide_zero) == 0) {
                    plane1 = plane2;
                }
                for (j = 0; j < count_new_edges; j++) {
                    if (new_edges[j].index1 == new_edges[i].index1 && new_edges[j].index2 == pVertex_index) {
                        new_edge_to_plane_indices[j].index2 = plane1;
                        break;
                    }
                }
                if (j >= count_new_edges) {
                    new_edges[count_new_edges].index1 = new_edges[i].index1;
                    new_edges[count_new_edges].index2 = pVertex_index;
                    new_edge_to_plane_indices[count_new_edges].index1 = plane1;
                    count_new_edges += 1;
                }
                for (j = 0; j < count_new_edges; j++) {
                    if (new_edges[j].index1 == new_edges[i].index2 && new_edges[j].index2 == pVertex_index) {
                        new_edge_to_plane_indices[j].index2 = plane1;
                        break;
                    }
                }
                if (j >= count_new_edges) {
                    new_edges[count_new_edges].index1 = new_edges[i].index2;
                    new_edges[count_new_edges].index2 = pVertex_index;
                    new_edge_to_plane_indices[count_new_edges].index1 = plane1;
                    count_new_edges += 1;
                }
    ...
    for (i = 0; i < pPolyhedron->count_planes; i++) {
        if ((plane_side_buffer[i] & ePlaneSide_negative) == 0) {
    ...
    for (i = 0; i < count_new_planes; i++) {
        ...
        for (j = 0; i < pPolyhedron-> count_edges; j++) {
...
tPhysicsError C2_HOOK_FASTCALL AddPolyhedronCollisionShapePlanes(tCollision_shape_polyhedron_data* pPolyhedron) {
    ...
    read_idx = 2;
    write_idx = 1;
    for (i = 0; i < pPolyhedron->count_points; ) {
        ...
        for (i = 0; i < pPolyhedron->count_points; i++) {
            for (j = 0; j < pPolyhedron->count_edges; j++) {
                ...
                if (i != new_count_points) {
                    ...
                    for (k = 0; k < pPolyhedron->count_edges; k++) {

                        if (pPolyhedron->edges[k].index1 == i) {
                            pPolyhedron->edges[k].index1 = new_count_points;
                        }
                        if (pPolyhedron->edges[k].index2 == i) {
                            pPolyhedron->edges[k].index2 = new_count_points;
                        }
                    }
                }
                new_count_points++;
