...
void C2_HOOK_FASTCALL RememberSafePosition(tCar_spec* car, tU32 pTime) {
    ...
    if (!car->car_crush_spec->field_0x4b8) {
        return;
    }
    for (j = 0; j < 4; j++) {
        if (car->susp_height[j / 2] <= car->oldd[j]) {
            return;
        }
    }
    if ((car->collision_info->last_special_volume == NULL || car->collision_info->last_special_volume->gravity_multiplier == 1.f)
        ...
        for (j = 0; j < REC2_ASIZE(car->last_safe_positions); j++) {
        ...
        for (j = REC2_ASIZE(car->last_safe_positions) - 1; j > 0; j--) {
...
//    int i;
    ...
    if (!car->keys.change_down) {
        ...
        if (car->number_of_wheels_on_ground < 3
                && BrVector3LengthSquared(&car->collision_info->v) < .4f
    ...
    C2_HOOK_BUG_ON(offsetof(tCar_spec, end_steering_damage_effect) != 5452);
