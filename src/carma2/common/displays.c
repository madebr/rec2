...
int C2_HOOK_FASTCALL DRTextCleverWidth(const tDR_font* pFont, const char* pText) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if ((tS8)pText[i] < 0) {
            ...
            result += GetPolyFontCharacterWidthI(pText[i], polyfont);
        }
    }
    return result;
}
...
void C2_HOOK_FASTCALL OoerrIveGotTextInMeBoxMissus(int pFont_index, const char* pText, br_pixelmap* pPixelmap, int pLeft, int pTop, int pRight, int pBottom, int pCentred) {
    ...
    polytext = C2V(gDRFont_to_polyfont_mapping)[C2V(gFonts)[pFont_index].id];
    RenderPolyText(polytext, pText, pLeft, pTop, pRight, pBottom, pCentred ? eJust_centre : eJust_left);
}
