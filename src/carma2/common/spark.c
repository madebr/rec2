...
C2_HOOK_VARIABLE_IMPLEMENT_INIT(int*, gShade_table, 0x00660144, &gShade_list[8]); /* FIXME: rename to gDust_table*/
...
void C2_HOOK_FASTCALL LoadInShrapnel(void) {
    ...
    C2V(gTrack_actor)->material = C2V(gBlack_material);
}
...
void C2_HOOK_FASTCALL InitFlame(void) {
    ...
    if (LoadTextureTryAllLocations(name, C2V(gFlame_map)) != REC2_ASIZE(C2V(gFlame_map))) {
...
void C2_HOOK_FASTCALL InitSplash(FILE* pF) {
    ...
    if (pF != NULL) {
...
#endif
            num_files = LoadTextureTryAllLocations(the_path, &splash_maps[C2V(gNum_splash_types)]);
            if (num_files == 0) {
        ...
        C2V(gNum_splash_types) = LoadTextureTryAllLocations(the_path, splash_maps);
    }
