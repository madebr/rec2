...
void C2_HOOK_FASTCALL RenderPolyText(int pFont, const char* pText, int pLeft, int pTop, int pRight, int pBottom, tJustification pJust) {
    char s[256];
    int s_len;
    int x;
    int y;
    int in_min_start_put;
    int in_start_put;
    int in_end_put;
    int in_pos;
    int newline;
    ...
    while (pText[in_pos] != '\0') {
        ...
        if (!newline) {
            ...
            in_start_put = in_min_start_put;
        }
        in_end_put = in_pos;
        if (max_width < text_x || newline) {
            int len_put;
            tJustification justif;

            for (; in_start_put < in_end_put && pText[in_end_put] != ' '; in_end_put--) {
            }
            ...
            for (; pText[in_start_put] == '\r' || pText[in_start_put] == ' '; in_start_put++) {
            }
            if (!C2V(gPolyFonts)[pFont].available) {
                LoadInterfacePolyFonts();
            }
            y += 2 * C2V(gPolyFonts)[pFont].fontCharacterHeight / 2;
            s_len = 0;
            text_x = 0;
            in_pos = in_start_put;
            in_min_start_put = in_start_put;
        } else {
            s_len++;
            in_pos++;
        }
...
void C2_HOOK_FASTCALL LoadPolyFont(const char* pName, int pSize, int pIndex) {
    ...
    for (i = 0; i < (int)C2V(gPolyFonts)[pIndex].numberOfCharacters; i++) {
        ...
        while (model != NULL) {
            if (model->bounds.max.v[0] == pSize && model->bounds.min.v[0] == -pSize) {
                break;
            }
            model = model->user;
        }
        if (model == NULL) {
            ...
            C2V(gPolyFonts)[pIndex].glyphs[pIndex].model->user = model;
            if (model == NULL) {
                FatalError(kFatalError_CouldNotCreateTexturesPages_S, pName);
            }
...
void C2_HOOK_FASTCALL LoadPolyFontWithTimerFix(int pFont, const char* pName, float pFactor, int pSize) {
    ...
    if (pFont == kPolyfont_ingame_big_timer) {
        ...
        C2V(gPolyFonts)[kPolyfont_ingame_big_timer].glyphs[58].glyph_width = 11; /* What is gyph 58? */
    }
}
...
void C2_HOOK_FASTCALL RenderPolyTextLine(const char *pText, int pX, int pY, int pFont, tJustification pJust, int pRender) {
    ...
    for (i = 0; i < text_len; i++) {
        ...
        draw_x += C2V(gPolyFonts)[pFont].glyphs[c].glyph_width;
    }
    if (pRender) {
