...
C2_HOOK_VARIABLE_IMPLEMENT(int, gS3_sample_filter_funcs_registered, 0x007a0758);
...
int C2_HOOK_FASTCALL S3ExecuteSampleFilterFuncs(tS3_channel* pChannel) {
    ...
    if (((pChannel->descriptor->effects_enabled == 0) & C2V(gS3_sample_filter_funcs_registered)) != 0) {
        C2V(gS3_sample_filter_func)(1, pChannel->tag);
        pChannel->descriptor->effects_enabled = 1;
    } else if (((C2V(gS3_sample_filter_funcs_registered) == 0) & pChannel->descriptor->effects_enabled) != 0) {
