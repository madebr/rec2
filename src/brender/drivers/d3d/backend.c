...
C2_HOOK_VARIABLE_IMPLEMENT_ARRAY(tStruct_100385e0, gCurrent_render_states, 100, 0x100385e0);
...
int C2_HOOK_CDECL DetectD3DDevices(void) {
    ...
    for (i = 0; i < BR_ASIZE(C2V(gCurrent_render_states)); i++) {
        C2V(gCurrent_render_states)[i].initialized = 0;
    }
                    ...
                    for (x = 0; x < surface_desc.dwWidth; x++, ptr++) {
                        if (buffer->pm_type == BR_PMT_RGBA_4444) {
                            ...
                            *ptr = ((c & 0xf00) << 3) | ((c & 0xf0) << 2) | ((c & 0xf) << 1);
                            ...
                            if (C2V(gTextures_alpha_init)[0][(((a ^ y) & 0xf) ^ a)][x & 0xf] == 0) {
                                *ptr = 0;
                            }
                        }
                    }
