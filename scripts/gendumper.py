            ...
            for p in prev_pending:
                match type(p.node):
                    case pycparser.c_ast.Struct:
                        with writer.dump_function(name=p.funcname, typename=p.typename, varname="s", pointer=True):
                            dumper.dump_body_Struct(p.node, varname="s", pointer=True)
                    case pycparser.c_ast.Union:
                        with writer.dump_function(name=p.funcname, typename=p.typename, varname="e", pointer=True):
                            dumper.dump_body_Union(p.node, varname="e", pointer=True)
                    case pycparser.c_ast.Enum:
                        with writer.dump_function(name=p.funcname, typename=p.typename, varname="e", pointer=False):
        ...
        match top_id_type_name:
            ...
            case _:
                try:
                    ...
                    self.writer.emit_printf(f"{key.fmt}: unknown type", key.args)
                    return
                try:
    ...
    def _dump_PtrDecl(self, node: pycparser.c_ast.TypeDecl, key: Key, varname: str):
        with self.writer.ifelse() as ifelse:
            with ifelse.if_(f"{varname} != NULL"):
                self._dump_item(node.type, key=key.add_string_literal('[0]'), varname=varname + "[0]")
            with ifelse.else_():
...
class TxtWriter(AbstractWriter):
    ...
    def for_loop(self, dim: str):
        ...
        self.cprint(f"{self._indent_str}for (unsigned int {index} = 0; i < {dim}; i++) {{")
        self._array_depth += 1
        with self.indent():
            yield index
        self._array_depth -= 1
    ...
    def dump_function(self, name: str, typename: str, varname: str, pointer: bool):
        ...
        self.hprint(f"\nvoid {name}(const {typename} {maybe_asterisk}{varname}, unsigned int max_depth);")
        self.cprint(f"\nvoid {name}(const {typename} {maybe_asterisk}{varname}, unsigned int max_depth) {{")
        with self.indent():
            with self.ifelse() as ifelse:
                with ifelse.if_("max_depth == 0"):
    ...
    def emit_printf(self, fmt, *args):
        ...
        if args_str:
            args_str = ", " + args_str
        self.cprint(f"{self._indent_str}printf(\"{fmt}\"{args_str});")

    def call_dumper_function(self, funcname: str, varname: str, deref: bool):
        maybe_ampersand = "&" if deref else ""
        self.cprint(f"{self._indent_str}{funcname}({maybe_ampersand}{varname}, max_depth - 1);")
    ...
    def dump_sint(self, key: Key, varname: str):
        self.cprint(f"{self._indent_str}printf(\".{key.fmt} = %d\\n\"{key.args}, {varname});")

    def dump_uint(self, key: Key, varname: str):
        self.cprint(f"{self._indent_str}printf(\".{key.fmt} = %u\\n\"{key.args}, {varname});")

    def dump_sshort(self, key: Key, varname: str):
        self.cprint(f"{self._indent_str}printf(\".{key.fmt} = %hd\\n\"{key.args}, {varname});")

    def dump_ushort(self, key: Key, varname: str):
        self.cprint(f"{self._indent_str}printf(\".{key.fmt} = %uhd\\n\"{key.args}, {varname});")

    def dump_schar(self, key: Key, varname: str):
        self.cprint(f"{self._indent_str}printf(\".{key.fmt} = %d\\n\"{key.args}, {varname});")

    def dump_uchar(self, key: Key, varname: str):
        self.cprint(f"{self._indent_str}printf(\".{key.fmt} = %ud\\n\"{key.args}, {varname});")

    def dump_float(self, key: Key, varname: str):
        self.cprint(f"{self._indent_str}printf(\".{key.fmt} = %g\\n\"{key.args}, {varname});")

    def dump_double(self, key: Key, varname: str):
        self.cprint(f"{self._indent_str}printf(\".{key.fmt} = %g\\n\"{key.fmt}, {varname});")

    def dump_null(self, key: Key):
        self.cprint(f"{self._indent_str}printf(\".{key.fmt} = NULL\\n\"{key.args});")
